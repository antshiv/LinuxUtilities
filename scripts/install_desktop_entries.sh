...
write_entry "linuxutilities-control-center.desktop" \
  ...
  "bash -lc 'if [ -x \"${BASE_DIR}/build/bin/linux_control_center\" ]; then exec \"${BASE_DIR}/build/bin/linux_control_center\" \"${BASE_DIR}\"; elif [ -x \"$HOME/Programs/bin/linux_control_center\" ]; then exec \"$HOME/Programs/bin/linux_control_center\" \"${BASE_DIR}\"; else echo \"linux_control_center (GTK4) not found. Build with ./build_linux_control_center.sh\"; fi'"
